...
import { Transaction, NOTHING_TO_PROVE, BALANCE_TRANSACTION_TO_PROVE, TRANSACTION_TO_PROVE, getImpureCircuitIds, SucceedEntirely } from '@midnight-ntwrk/midnight-js-types';
import 'fs';
import 'path';
import { ChargedState as ChargedState$1 } from '@midnight-ntwrk/onchain-runtime-v1';
 ...
 * limitations under the License.
 */
const toLedgerContractState = (contractState) => {
    const serialized = contractState.serialize();
    const v5Header = 'midnight:contract-state[v5]:';
    const v4Header = 'midnight:contract-state[v4]:';
    const headerStr = String.fromCharCode(...serialized.slice(0, v5Header.length));
    if (headerStr.startsWith(v5Header)) {
        // Convert v5 header to v4 for compatibility with ledger-v6@6.1.0-alpha.6
        const v4Serialized = Buffer.concat([Buffer.from(v4Header), serialized.slice(v5Header.length)]);
        return ContractState.deserialize(v4Serialized);
    }
    return ContractState.deserialize(serialized);
};
...
class TransactionContextImpl {
    ...
    [MergeUnsubmittedCallTxData](circuitId, callData, privateStateId) {
        ...
        contractState.data = new ChargedState$1(callData.public.nextContractState);
...
async function createUnprovenCallTx(providers, options, transactionContext) {
    ...
    if (hasPrivateStateId && hasPrivateStateProvider) {
        ...
        return createUnprovenCallTxFromInitialStates(createCallOptions(options, parseCoinPublicKeyToHex(providers.walletProvider.getCoinPublicKey(), getNetworkId()), contractState, zswapChainState, privateState), providers.walletProvider.getCoinPublicKey(), providers.walletProvider.getEncryptionPublicKey());
    }
...
const createCircuitCallTxInterface = (providers, contract, contractAddress, privateStateId) => {
    ...
    return getImpureCircuitIds(contract).reduce((acc, circuitId) => ({
        ...
        [circuitId]: (...args) => {
            ...
            const callOptions = createCallTxOptions(contract, circuitId, contractAddress, privateStateId, callArgs);
            return txCtx
