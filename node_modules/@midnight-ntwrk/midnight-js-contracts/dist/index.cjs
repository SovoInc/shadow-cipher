...
require('fs');
require('path');
var onchainRuntimeV1 = require('@midnight-ntwrk/onchain-runtime-v1');
...
async function proveTransaction(recipe, providers, proveTxConfig) {
    let toSubmit;
    switch (recipe.type) {
        case midnightJsTypes.TRANSACTION_TO_PROVE: {
            toSubmit = await providers.proofProvider.proveTx(recipe.transaction, proveTxConfig);
            break;
        }
        case midnightJsTypes.BALANCE_TRANSACTION_TO_PROVE: {
        ...
        case midnightJsTypes.NOTHING_TO_PROVE: {
...
async function createUnprovenDeployTx(providers, options) {
    ...
    return createUnprovenDeployTxFromVerifierKeys(verifierKeys, midnightJsUtils.parseCoinPublicKeyToHex(providers.walletProvider.getCoinPublicKey(), midnightJsNetworkId.getNetworkId()), options, providers.walletProvider.getEncryptionPublicKey());
}
...
class TransactionContextImpl {
    ...
    [MergeUnsubmittedCallTxData](circuitId, callData, privateStateId) {
        ...
        contractState.data = new onchainRuntimeV1.ChargedState(callData.public.nextContractState);
        const [zswapChainState] = callData.private.unprovenTx.guaranteedOffer
            ? this.currentStates.zswapChainState.tryApply(ledgerV6.ZswapOffer.deserialize('pre-proof', callData.private.unprovenTx.guaranteedOffer.serialize()))
...
async function createUnprovenCallTx(providers, options, transactionContext) {
    ...
    if (hasPrivateStateId && hasPrivateStateProvider) {
        ...
        return createUnprovenCallTxFromInitialStates(createCallOptions(options, midnightJsUtils.parseCoinPublicKeyToHex(providers.walletProvider.getCoinPublicKey(), midnightJsNetworkId.getNetworkId()), contractState, zswapChainState, privateState), providers.walletProvider.getCoinPublicKey(), providers.walletProvider.getEncryptionPublicKey());
    }
    const { zswapChainState, contractState } = await getContractPublicStates(providers, options, transactionContext);
    return createUnprovenCallTxFromInitialStates(createCallOptions(options, midnightJsUtils.parseCoinPublicKeyToHex(providers.walletProvider.getCoinPublicKey(), midnightJsNetworkId.getNetworkId()), contractState, zswapChainState), providers.walletProvider.getCoinPublicKey(), providers.walletProvider.getEncryptionPublicKey());
}
