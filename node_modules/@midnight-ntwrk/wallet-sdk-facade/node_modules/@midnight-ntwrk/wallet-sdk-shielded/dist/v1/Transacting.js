...
export const makeSimulatorTransactingCapability = (config, getContext) => {
    return new TransactingCapabilityImplementation(config.networkId, () => getContext().coinSelection, () => getContext().coinsAndBalancesCapability, () => getContext().keysCapability, TransactionTrait.proofErased);
};
class NoSelfOutputsError extends Data.TaggedError('NoSelfOutputs') {
}
export class TransactingCapabilityImplementation {
    ...
    balanceTransaction(secretKeys, state, tx) {
        return Either.gen(this, function* () {
            ...
            const { newState: afterGuaranteed, offer: guaranteed } = yield* this.#balanceGuaranteedSection(secretKeys, afterFallible, initialImbalances, coinSelection, 0, Imbalances.empty());
            return {
    ...
    makeTransfer(secretKeys, state, transfers) {
        return Either.gen(this, function* () {
            ...
            const { offer, newState } = yield* this.#balanceGuaranteedSection(secretKeys, state, imbalances, this.getCoinSelection(), selfCoins.length, Imbalances.empty());
    ...
    initSwap(secretKeys, state, desiredInputs, desiredOutputs) {
        return Either.gen(this, function* () {
            ...
            const networkId = this.networkId;
            const { offer, newState } = yield* this.#balanceGuaranteedSection(secretKeys, state, TransactionImbalances.empty(), this.getCoinSelection(), outputsParseResult.selfCoins.length, inputsParseResult);
    ...
    #balanceGuaranteedSection(secretKeys, state, imbalances, coinSelection, knownSelfOutputs, targetImbalances) {
        return Either.gen(this, function* () {
        ...
        }).pipe(EitherOps.flatMapLeft((err) => {
    ...
    #balanceGuaranteedWithSelfOutput(secretKeys, state, imbalances, coinSelection, targetImbalances) {
        return Either.gen(this, function* () {
            const additionalOutputValue = 100000n;
            const balanceRecipe = yield* Either.try({
                try: () => {
                    return getBalanceRecipe({
                        coins: this.getCoins()
                            .getAvailableCoins(state)
                            .map((c) => c.coin),
                        initialImbalances: Imbalances.merge(imbalances.guaranteed, Imbalances.fromEntry(ledger.shieldedToken().raw, -1n * additionalOutputValue)),
                        transactionCostModel: ShieldedCostModel,
                        feeTokenType: '',
                        coinSelection,
                        targetImbalances,
            ...
            const additionalCoin = ledger.createShieldedCoinInfo(ledger.shieldedToken().raw, additionalOutputValue);
            const additionalOffer = pipe(additionalCoin, (coin) => ledger.ZswapOutput.new(coin, 0, this.getKeys().getCoinPublicKey(state).toHexString(), this.getKeys().getEncryptionPublicKey(state).toHexString()), (output) => ledger.ZswapOffer.fromOutput(output, ledger.shieldedToken().raw, additionalOutputValue));
            return yield* pipe(this.#prepareOffer(secretKeys, state, balanceRecipe, 0), Option.map(({ newState, offer }) => {
                return {
