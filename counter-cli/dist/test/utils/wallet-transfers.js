...
export async function sendUnshieldedToken(wallet, address, amount) {
    ...
    const transferRecipe = await wallet.wallet.transferTransaction(wallet.shieldedSecretKeys, wallet.dustSecretKey, tokenTransfer, txTtl);
    const signedTx = await wallet.wallet.signTransaction(transferRecipe.transaction, (payload) => wallet.unshieldedKeystore.signData(payload));
...
export async function sendArbitraryUnshieldedToken(wallet, address, amount) {
    ...
    const recipe = await wallet.wallet.balanceTransaction(wallet.shieldedSecretKeys, wallet.dustSecretKey, arbitraryTx, new Date(Date.now() + 30 * 60 * 1000));
    if (recipe.type !== 'TransactionToProve') {
        throw new Error('Expected a transaction to prove');
    }
    const signedTx = await wallet.wallet.signTransaction(recipe.transaction, (payload) => wallet.unshieldedKeystore.signData(payload));
    const finalizedTx = await wallet.wallet.finalizeTransaction({
        ...recipe,
        transaction: signedTx,
    });
